...
function get_menu_param(name) {
  ...
  if (!param_str || param_str == "") return null;
...
function top_menu() {
  ...
  $.each(menu_list, function(i, item_obj) {
    ...
    } else item = '<li><span><span><a href="#" id="top_menu_' + item_obj.id + '">' + item_obj.name + "</a></span></span>";
...
function left_menu(id) {
  ...
  $.each(item_obj.child_menu, function(j, item_obj_child) {
    $("#left-column").append(
      '<a href="#" class="link">' + item_obj_child.name + "</a>"
    );

    if (j == 0)
      $("#left-column").append(
        '<ul class="nav" id="child_menu_' + item_obj_child.id + '"></ul>'
      );
    //默认选定第一个
    else
      $("#left-column").append(
        '<ul class="navhide" id="child_menu_' + item_obj_child.id + '"></ul>'
      );
    ...
    $.each(item_obj_child.child_menu, function(k, item_obj_child2) {
      ...
      if (k + 1 < item_obj_child.child_menu.length)
        $("#child_menu_" + item_obj_child.id).append(
          '<li><a href="#" id="child2_menu_' +
            item_obj_child2.id +
            '">' +
            item_obj_child2.name +
            "</a></li>"
        );
      else
        $("#child_menu_" + item_obj_child.id).append(
          '<li class="last"><a href="#" id="child2_menu_' +
            item_obj_child2.id +
            '">' +
            item_obj_child2.name +
            "</a></li>"
...
$(function() {
  //上面的菜单
  $.getJSON("ajax_menu_list", function(data) {
    menu_list = data.data;
  ...
  $("#logout").click(function() {
    if (confirm("确认退出吗？")) {
      $.getJSON("ajax_logout", function(data) {
        location.href = "index";
      });
    }
    return false;
